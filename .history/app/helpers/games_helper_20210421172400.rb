  ...
  def game_display(games, external)
    ...
    games.each do |x|
      @row << content_tag(:tr) do
        ...
        unless external
          @table << content_tag(:td, class: 'main-text align-middle justify-content-center d-flex') do
            image_tag("#{x.genres.first.icon}.png", size: '50x50', class: 'd-flex align-self-center') unless x.genres.empty?
          end
        end
